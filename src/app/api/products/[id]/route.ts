  ...
  try {
    ...
    try {
      ...
      if (keysToDelete.length > 0) {
        ...
        console.log({ message: 'Cache invalidated for product', productId, keysToDelete });
      }
      ...
      console.warn({ message: 'Redis cache invalidation error', productId, redisError });
    }

    return NextResponse.json({ message: `Produk "${product.name}" berhasil dihapus.` }, { status: 200 });

  } catch (error: any) {
    console.error({ message: 'Error deleting product', productId, error });
...
export async function GET(
  ...
  try {
    ...
    if (productWithCombinations.hasVariants && productWithCombinations.combinations && productWithCombinations.combinations.length > 0) {
      ...
      if (!cachedDataString) {
        ...
        try {
          ...
          console.warn({ message: 'Redis SET error', cacheKey, redisSetError });
        }
      }
    ...
    console.error({ message: 'Error fetching product', productId, error });
